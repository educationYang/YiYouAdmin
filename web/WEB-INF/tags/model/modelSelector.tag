...
function ${id}_show(paramData)
{
	...
	if(!(supplierSelector_dialog_${id}.dialog("isOpen")==true||supplierSelector_dialog_${id}.dialog("isOpen")==false)){
		...
		fillDivWithPage("supplierSelector_divDlg${id}",__ctxPath+"/selector/modelSelector.html",getSupplierSelectorData_${id}(),"post");
...
function fn${id}GetData(){
	...
	fillDivWithPage("supplierSelectorDataList_${id}","${ctxPath}/selector/modelSelectorDataList.html?pagingId=${id}&culId=${culId}",paraDate${getDataCallBack},"post");
}
...
<c:choose>
	<c:when test="${not empty multiSelect&&multiSelect}">
		...
		if(val){
				...
				var supplier=__fnGetObjJsonData_${id}(pid);
				fnAddSelectedSupplier${id}(supplier);
				$j("#sel_ch_${id}_"+supplier.modelId).attr("checked",true);
			}
			
		}
		function fnAddSelectedSupplier${id}(supplier){
		  // alert("双击添加");
		  // alert("supplierID:"+supplier.modelId)
			var exist=false;
			for(var i=0;i<selectedSupplierList${id}.length;i++){
				if(selectedSupplierList${id}[i].supplierId==supplier.modelId)
					exist=true;
			}
		//	alert("exist:"+exist);
			if(!exist){
			//    alert("exit");
				...
				$j("#selectedSupplier_${id}").append("<span id='sel_${id}_"+supplier.modelId+"' ondblclick='fnRemoveSelectedSupplier${id}("+supplier.modelId+")' title='双击移除'>"+supplier.title+",</span>");
			}
		}
		...
		function fnRemoveSelectedSupplier${id}(id){
		   // alert("移除所选:"+id);
		//	for(var i=0;i<selectedSupplierList${id}.length;i++){
			//	if(selectedSupplierList${id}[i].modelId==id){
